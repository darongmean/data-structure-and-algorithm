(ns adwh.exhausted-search.depth-first-search.lunar-landing-puzzle
  "
   ...
   The frontier – the list of paths waiting to be explored further – is maintained as a stack,
   with new entries added to the front of the stack.
   ...
   - this algorithm is faster than breadth first search when finding the first solution
   - this algorithm is slower than breadth first search when finding the shortest solution
  "
  (:require
  ...
  (let [mk-stack (fn [coll]
    ...
    (loop [stack (mk-stack paths)
      ...
      (cond
        ...
        (solved-path (peek stack))
        (recur (pop stack)
               (conj result (:simple-moves (peek stack))))
  ...
  (let [mk-stack (fn [coll]
    ...
    (loop [stack (mk-stack paths)
      ...
      (cond
        ...
        (not-simple-move? intermediate-states (peek stack))
        (recur (pop stack) intermediate-states)

        :else
        (recur (push-all (pop stack)
                         (succs (peek stack)))
               (conj intermediate-states (:state (peek stack))))))))
